...
sed 's/atg/M/g' compiled_ORFs.txt | sed 's/gca\|gcc\|gcg\|gct/A/g' | sed 's/tgc\|tgt/C/g' | sed 's/gac\|gat/D/g' | sed 's/gaa\|gag/E/g' |  sed 's/ttc\|ttt/F/g' | sed 's/gga\|ggc\|ggg\|ggt/G/g' | sed 's/cac\|cat/H/g' | sed 's/ata\|atc\|att/I/g' | sed 's/aaa\|aag/K/g' | sed 's/cta\|ctc\|ctg\|ctt\|tta\|ttg/L/g' | sed 's/aac\|aat/N/g' | sed 's/cca\|ccc\|ccg\|cct/P/g' | sed 's/caa\|cag/O/g' | sed 's/aga\|agg\|cga\|cgc\|cgg\|cgt/O/g' | sed 's/agc\|agt\|tca\|tcc\|tcg\|tct/S/g' | sed 's/aca\|acc\|acg\|act/T/g' | sed 's/gta\|gtc\|gtg\|gtt/V/g' | sed 's/tgg/W/g' | sed 's/tac\|tat/Y/g' | sed 's/taa\|tag\|tga/*/g' | tr -d ' ' > translated_ORFs.txt

# formatting compiled_ORFs.txt by removing spaces
cat compiled_ORFs0.txt | tr -d ' ' > compiled_ORFs.txt
...
echo -e 'fna formatted file outputted to: compiled_ORFs.txt'
echo -e 'faa formaated file outputted to: translated_ORFs.txt'
